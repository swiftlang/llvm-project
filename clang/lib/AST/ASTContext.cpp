...
ASTContext::ASTContext(LangOptions &LOpts, SourceManager &SM,
    ...
    : ConstantArrayTypes(this_(), ConstantArrayTypesLog2InitSize),
      ...
      HLSLAttributedResourceTypes(this_()), ValueTerminatedTypes(this_()),
...
static QualType assureMandatorySugarTypesRemain(const ASTContext &Ctx,
                                                QualType DestTy,
                                                QualType SrcTy) {
...
public:
  ...
  RetTy VisitTypedefType(const TypedefType *T) {
    ...
    if (NewUnderlyingTy == UnderlyingTy) {
      QualType NewTy = Ctx.getTypedefType(T->getKeyword(), T->getQualifier(), T->getDecl(), UnderlyingTy);
      return {NewTy, false};
    }
...
QualType ASTContext::getPointerType(QualType T,
  ...
  // structure.
  llvm::FoldingSetInsertToken Token;
  if (PointerType *PT = PointerTypes.lookup({T, A}, Token))
  ...
  if (!T.isCanonical()) {
    ...
    assert(!PointerTypes.lookup({T, A}, Token) && "Shouldn't be in the map!");
  }
...
void ASTContext::adjustObjCTypeParamBoundType(const ObjCTypeParamDecl *Orig,
  ...
  if (const Type *TypeForDecl = New->TypeForDecl) {
