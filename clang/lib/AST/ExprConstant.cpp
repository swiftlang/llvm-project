...
namespace {
  ...
  private:
    ...
    ASTContext &getASTCtx() const override { return Ctx; }
    ...
    bool hasDiagnostics() const override { return !EvalStatus.Diag; }

    bool inConstantContext() const override { return InConstantContext; }
  ...
  public:
    ...
    LLVM_NODISCARD bool noteFailure() override {
...
static CharUnits getBaseAlignment(EvalInfo &Info, const LValue &Value) {
  ...
  if (const auto *E = Value.Base.dyn_cast<const Expr *>())
    return interp::getAlignOfExpr(Info.Ctx, E, UETT_AlignOf);
  return interp::getAlignOfType(Info.Ctx, Value.Base.getTypeInfoType(), UETT_AlignOf);
}
...
EvaluateBuiltinClassifyType(const CallExpr *E, const LangOptions &LangOpts) {
  ...
  if (E->getNumArgs() == 0)
    return interp::GCCTypeClass::None;
  ...
  // argument.
  return interp::evaluateBuiltinClassifyType(E->getArg(0)->getType(), LangOpts);
}
...
bool IntExprEvaluator::VisitUnaryExprOrTypeTraitExpr(
  ...
  case UETT_AlignOf: {
    if (E->isArgumentType())
      return Success(
          interp::getAlignOfType(Info.Ctx, E->getArgumentType(), E->getKind()),
          E);
    else
      return Success(
          interp::getAlignOfExpr(Info.Ctx, E->getArgumentExpr(), E->getKind()),
          E);
  }
