...
void FileIndexRecord::removeHeaderGuardMacros() {
  auto It = std::remove_if(Decls.begin(),Decls.end(), [](const DeclOccurrence &D) {
    if (auto *MI = D.DeclOrMacro.dyn_cast<const MacroInfo *>())
...
void FileIndexRecord::print(llvm::raw_ostream &OS, SourceManager &SM) const {
  ...
  for (auto &DclInfo : Decls) {
    if (auto *D = DclInfo.DeclOrMacro.dyn_cast<const Decl *>()) {
      ...
      auto *MI = DclInfo.DeclOrMacro.get<const MacroInfo *>();
