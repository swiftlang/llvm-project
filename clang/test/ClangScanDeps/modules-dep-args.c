...
// RUN: cat %t/result_cache.json  | sed 's:\\\\\?:/:g' | FileCheck %s -DPREFIX=%/t -DSUBMODULE_PREFIX=%{/t:real} --check-prefixes=CHECK,CHECK_CACHE
...
// RUN: cat %t/result_build.json | sed 's:\\\\\?:/:g' | FileCheck %s -DPREFIX=%/t -DSUBMODULE_PREFIX=%{/t:real} --check-prefixes=CHECK,CHECK_BUILD
...
// RUN: cat %t/result_lazy.json | sed 's:\\\\\?:/:g' | FileCheck %s -DPREFIX=%/t -DSUBMODULE_PREFIX=%{/t:real} --check-prefixes=CHECK,CHECK_LAZY
...
// RUN: cat %t/result_eager.json | sed 's:\\\\\?:/:g' | FileCheck %s -DPREFIX=%/t -DSUBMODULE_PREFIX=%{/t:real} --check-prefixes=CHECK,CHECK_EAGER
...
// CHECK_LAZY:         "-fmodule-map-file=[[SUBMODULE_PREFIX]]/module.modulemap"
