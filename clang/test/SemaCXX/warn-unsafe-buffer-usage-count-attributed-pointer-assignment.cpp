...
void immutable_ptr_to_ptr(int *__counted_by(*count) *p, int *count) {
  p = nullptr; // expected-warning{{cannot assign to parameter 'p' because it points to a bounds-attributed pointer}}
  *count = 0;
}

void immutable_ptr_to_value(int *__counted_by(*count) *p, int *count) {
  *p = nullptr;
  count = nullptr; // expected-warning{{cannot assign to parameter 'count' because it points to a dependent value}}
}

void immutable_ptr_with_inout_value(int *__counted_by(*count) p, int *count) {
  p = nullptr; // expected-warning{{cannot assign to parameter 'p' because its type depends on an inout dependent value}}
  *count = 0;
}
...
void immutable_value_with_inout_ptr(int *__counted_by(count) *p, int count) {
  *p = nullptr;
  count = 0; // expected-warning{{cannot assign to parameter 'count' because it's used as dependent value in an inout bounds-attributed pointer}}
}
...
// Standalone assigns to bounds-attributed

void standalone_assign_to_ptr(int *__counted_by(count) p, int count, int *q) {
  q = p;
  q = p = q;     // expected-warning{{assignment to bounds-attributed pointer 'p' must be inside of a bounds-attributed group in a compound statement}}
  q = q = p = q; // expected-warning{{assignment to bounds-attributed pointer 'p' must be inside of a bounds-attributed group in a compound statement}}

  // FIXME: Duplicated error message + the error message doesn't make sense.
  p++; // expected-warning{{unsafe pointer arithmetic}} expected-warning{{assignment to bounds-attributed pointer 'p' must be inside of a bounds-attributed group in a compound statement}} expected-note{{pass -fsafe-buffer-usage-suggestions to receive code hardening suggestions}}
}

void standalone_assign_to_count(int *__counted_by(count) p, int count, int n) {
  n = count;
  n = count = n;     // expected-warning{{assignment to dependent value 'count' must be inside of a bounds-attributed group in a compound statement}}
  n = n = count = n; // expected-warning{{assignment to dependent value 'count' must be inside of a bounds-attributed group in a compound statement}}

  // FIXME: The error message doesn't make sense.
  count++;         // expected-warning{{assignment to dependent value 'count' must be inside of a bounds-attributed group in a compound statement}}
  --count;         // expected-warning{{assignment to dependent value 'count' must be inside of a bounds-attributed group in a compound statement}}
  count += 42;     // expected-warning{{assignment to dependent value 'count' must be inside of a bounds-attributed group in a compound statement}}
  count -= 42;     // expected-warning{{assignment to dependent value 'count' must be inside of a bounds-attributed group in a compound statement}}
  n = n + count++; // expected-warning{{assignment to dependent value 'count' must be inside of a bounds-attributed group in a compound statement}}
}
