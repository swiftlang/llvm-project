...
// The LateBoundsAttrDiagContext walker used to reject this up front; now we
// emit the equivalent diagnostic during descent instead of asserting.
//
// For the array shape used here, -fexperimental-bounds-safety-attributes reaches
// the crashing TraverseArrayType path (checked by the 'expected' directives),
// while -fbounds-safety rejects the same source earlier, before type
// construction, with a different diagnostic (checked by the 'bounds-safety'
// directives). The two RUN lines pin both behaviours; this is why the crash
// reproducer uses -fexperimental-bounds-safety-attributes.
//
// FIXME: The diagnostics checked under -fexperimental-bounds-safety-attributes
// have misleading wording, tracked separately: the "pointer to incomplete
// __counted_by array type" message on a complete array (rdar://184258376) and
// the "multiple coupled declarations" message (rdar://184258982). Update the
// diagnostic expectations here when those are addressed.

#include <ptrcheck.h>

// '__counted_by' on the element of the (decayed) array.
// expected-error@+3{{pointer to incomplete __counted_by array type 'int *[10]' not allowed; did you mean to use a nested pointer type?}}
