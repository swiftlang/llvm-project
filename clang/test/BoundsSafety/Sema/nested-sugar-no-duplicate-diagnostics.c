...
// sugar one layer at a time and re-runs the Level-0 leaf check on each
// desugared layer. A leaf check that diagnoses must fire exactly once when the
// attribute target is reached through several layers of sugar (nested
// typedefs) -- not once per layer. Every declaration below expects exactly one
// diagnostic; a duplicate would make -verify fail. All bounds attributes are
// covered.
