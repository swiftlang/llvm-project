...
// *name* -- a typedef, __typeof__, or (in C++) a using-alias. Otherwise the
// attribute is silently sunk onto the function's return pointer (Apple clang
// rejects this; swiftlang stable/23.x asserts). The rejection lives in
// ConstructDynamicBoundType::VisitType, which is reached only for such "naming"
// sugar: a directly-written function type -- bare, or wrapped in *transparent*
// declarator sugar (parentheses, a calling-convention attribute, or an
// attribute spelled via a macro) -- dispatches to its own visitor instead and
// stays valid, so a bounds attribute there still legitimately applies to the
// return type. The check uses the canonical type, so it also sees through an
// AttributedType / MacroQualifiedType sitting between the name and the function.
