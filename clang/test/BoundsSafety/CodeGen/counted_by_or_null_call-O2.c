...
// CHECK-NEXT:    [[CMP_NOT159:%.*]] = icmp slt i32 [[TMP1]], 0, !annotation [[META7]]
// CHECK-NEXT:    [[CMP_NOT:%.*]] = select i1 [[DOTNOT]], i1 [[CMP_NOT159]], i1 false, !annotation [[META7]]
// CHECK-NEXT:    br i1 [[CMP_NOT]], label [[TRAP:%.*]], label [[LAND_RHS:%.*]], !annotation [[META7]]
// CHECK:       trap:
// CHECK-NEXT:    call void @llvm.ubsantrap(i8 25) #[[ATTR6]], !annotation [[META16:![0-9]+]]
// CHECK-NEXT:    unreachable, !annotation [[META16]]
// CHECK:       land.rhs:
// CHECK-NEXT:    br i1 [[DOTNOT]], label [[LOR_RHS:%.*]], label [[CONT133:%.*]], !annotation [[META7]]
// CHECK:       lor.rhs:
// CHECK-NEXT:    [[CMP56:%.*]] = icmp sge i32 [[TMP1]], [[LEN]], !annotation [[META7]]
// CHECK-NEXT:    [[CMP59:%.*]] = icmp sgt i32 [[LEN]], -1, !annotation [[META7]]
// CHECK-NEXT:    [[SPEC_SELECT:%.*]] = and i1 [[CMP59]], [[CMP56]]
// CHECK-NEXT:    br i1 [[SPEC_SELECT]], label [[CONT133]], label [[TRAP]], !prof [[PROF13]], !annotation [[META7]]
// CHECK:       cont133:
