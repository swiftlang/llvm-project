...
TEST(OnDiskCASTest, StandaloneMemoryBufferSurvivesPruning) {
  // The point of a standalone buffer is that lldb-style clients can drop the
  // store, letting its lock go so the CAS can be pruned, and still read what
  // they loaded. Check that pruning the storage out from under the buffer --
  // which is what the released lock allows -- leaves it readable.
  unittest::TempDir Temp("on-disk-cas", /*Unique=*/true);
  ...
  auto OpenCAS = [&Temp](std::unique_ptr<ObjectStore> &CAS) -> Error {
    std::pair<std::unique_ptr<ObjectStore>, std::unique_ptr<ActionCache>> DBs;
    if (Error E = createOnDiskUnifiedCASDatabases(Temp.path()).moveInto(DBs))
      return E;
    CAS = std::move(DBs.first);
  ...
  {
    std::unique_ptr<ObjectStore> CAS;
    ASSERT_THAT_ERROR(OpenCAS(CAS), Succeeded());

    std::optional<ObjectProxy> Small, Big;
    ASSERT_THAT_ERROR(CAS->createProxy({}, SmallData).moveInto(Small),
                      Succeeded());
    ASSERT_THAT_ERROR(CAS->createProxy({}, BigData).moveInto(Big), Succeeded());
    SmallBuffer = Small->getStandaloneMemoryBuffer();
    BigBuffer = Big->getStandaloneMemoryBuffer();
    ASSERT_TRUE(SmallBuffer);
    ASSERT_TRUE(BigBuffer);
  }

  // A tiny size limit makes each close start a fresh primary directory, so the
  // data written above becomes garbage after two rounds; then collect it.
  for (int I = 0; I < 2; ++I) {
    std::unique_ptr<ObjectStore> CAS;
    ASSERT_THAT_ERROR(OpenCAS(CAS), Succeeded());
...
TEST(OnDiskCASTest, StandaloneMemoryBufferSurvivesDeletedCAS) {
  // Stronger than pruning: delete the whole CAS directory. A mapping keeps the
  // file alive until it is unmapped, so the bytes have to stay readable.
  unittest::TempDir Temp("on-disk-cas", /*Unique=*/true);
  ...
  {
    std::pair<std::unique_ptr<ObjectStore>, std::unique_ptr<ActionCache>> DBs;
    ASSERT_THAT_ERROR(createOnDiskUnifiedCASDatabases(Temp.path()).moveInto(DBs),
                      Succeeded());
    std::optional<ObjectProxy> Small, Big;
...
TEST(OnDiskCASTest, StandaloneMemoryBufferRecordWithRefs) {
