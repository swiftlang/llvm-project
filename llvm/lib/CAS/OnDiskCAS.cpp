...
CASID OnDiskCAS::getID(ObjectHandle Handle) const {
  llvm_unreachable("unimplemented");
}

Expected<CASID> OnDiskCAS::parseIDImpl(ArrayRef<uint8_t> Hash) {
  llvm_unreachable("unimplemented");
}

Optional<ObjectRef> OnDiskCAS::getReference(const CASID &ID) const {
  Optional<ondisk::ObjectID> ObjID =
...
Expected<ObjectHandle> OnDiskCAS::load(ObjectRef ExternalRef) {
  Expected<Optional<ondisk::ObjectHandle>> ObjHnd =
