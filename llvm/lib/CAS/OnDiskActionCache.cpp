...
public:
  ...
  InMemoryActionCache() = delete;
  explicit InMemoryActionCache(const Namespace &NS, InMemoryOnlyTag)
      : CASDB(BuiltinNamespace::get()) {
    AlignedInMemoryStrings.emplace();
  }
  explicit InMemoryActionCache(const Namespace &NS,
                              std::shared_ptr<OnDiskHashMappedTrie> OnDiskResults)
      : ActionCache(BuiltinNamespace::get()),
...
public:
  ...
  struct InMemoryOnlyTag {};
  BuiltinActionCache() = delete;
  explicit BuiltinActionCache(const Namespace &NS, InMemoryOnlyTag)
      : CASDB(BuiltinNamespace::get()) {
    AlignedInMemoryStrings.emplace();
  }
  explicit BuiltinActionCache(const Namespace &NS,
                              std::shared_ptr<OnDiskHashMappedTrie> OnDiskResults)
      : ActionCache(BuiltinNamespace::get()),
        OnDiskResults(std::move(OnDiskResults)) {
  }
