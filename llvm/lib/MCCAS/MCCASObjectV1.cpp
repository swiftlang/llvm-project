...
Expected<uint64_t> GroupRef::materialize(MCCASReader &Reader,
  ...
  for (auto ID : *Refs) {
    ...
    if (auto AbbrevRef = DebugAbbrevSectionRef::Cast(*Node)) {
      auto FragmentSize = materializeAbbrevFromTagImpl(
          Reader, *AbbrevRef, ArrayRef<cas::ObjectRef>(*Refs));
      if (!FragmentSize)
...
getAndSetDebugAbbrevOffsetAndSkip(MutableArrayRef<char> CUData,
  ...
  if (NewOffset.has_value()) {
    ...
    auto UnsignedData = MutableArrayRef<uint8_t>(
...
template <typename SectionRefTy> Error MCCASBuilder::finalizeSection() {
...
/// * If abbreviation_index is getEndOfDIESiblingsMarker(), raw_data is empty
/// and this denotes the end of a sequence of sibling DIEs.
...
mccasformats::v1::loadDIETopLevel(DIETopLevelRef TopLevelRef) {
  ...
  return LoadedDIETopLevel{std::move(AbbrevData), *DistinctData, *RootDIE};
}
