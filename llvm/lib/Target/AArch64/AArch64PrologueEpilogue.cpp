...
void AArch64PrologueEpilogueCommon::fixupCalleeSaveRestoreToFPBased(
    MachineInstr &MI, int64_t FPSPOffset) const {
...
void AArch64EpilogueEmitter::emitEpilogue() {
  ...
  if (MBB.end() != EpilogueEndI) {
    ...
    IsSwiftCoroPartialReturn = EpilogueEndI->getOpcode() == AArch64::RET_POPLESS;
  }
  ...
  if (IsSwiftCoroPartialReturn) {
    ...
    assert(EpilogueEndI->getOpcode() == AArch64::RET_POPLESS);
    BuildMI(MBB, EpilogueEndI, DL, TII->get(AArch64::RET_ReallyLR))
        .setMIFlag(MachineInstr::FrameDestroy);
    MBB.erase(EpilogueEndI);
    EpilogueEndI = MBB.getLastNonDebugInstr();
  }
