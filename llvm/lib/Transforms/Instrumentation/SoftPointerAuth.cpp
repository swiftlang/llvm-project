...
bool SoftPointerAuth::transformCall(CallInst *call) {
  ...
  auto intrinsic = callee->getIntrinsicID();
...
bool SoftPointerAuth::transformPointerAuthCall(CallBase *oldCall,
  ...
  CallBase *newCall;
  auto oldInvoke = dyn_cast_or_null<InvokeInst>(oldCall);
  if (oldInvoke != nullptr) {
    newCall = builder.CreateInvoke(oldInvoke->getFunctionType(), callee,
                                   oldInvoke->getNormalDest(),
                                   oldInvoke->getUnwindDest(),
                                   args, opBundles);
  } else {
    newCall = builder.CreateCall(oldCall->getFunctionType(), callee, args, opBundles);
  }
