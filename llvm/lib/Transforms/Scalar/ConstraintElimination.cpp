...
void State::addInfoFor(BasicBlock &BB) {
  ...
  if (canAddSuccessor(BB, Br->getSuccessor(0))) {
    if (auto *CmpI = dyn_cast<ICmpInst>(Br->getCondition()))
...
/* TO_UPSTREAM(BoundsSafety) OFF*/
  } else if (auto ImpliedCondition = TryWithConstraint(R)) {
    return ImpliedCondition;
  }
...
static bool checkAndReplaceCondition(
  ...
  auto ReplaceCmpWithConstant = [&](Instruction *CheckInst, bool IsTrue) {
    ...
    bool Changed = false;
    CheckInst->replaceUsesWithIf(ConstantC, [&DT, NumIn, NumOut, ContextInst,
                                             &Changed](Use &U) {
      auto *UserI = getContextInstForUse(U);
      auto *DTN = DT.getNode(UserI->getParent());
