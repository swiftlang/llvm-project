...
partitionCUDie(InMemoryCASDWARFObject::PartitionedDebugInfoSection &SplitData,
  ...
  while (Child && Child.getAbbreviationDeclarationPtr()) {
    auto NewName = partitionCUDie(SplitData, Child, DebugInfoData, CUOffset,
...
InMemoryCASDWARFObject::splitUpCUData(ArrayRef<char> DebugInfoData,
  ...
  if (DWARFDie CUDie = DCU.getUnitDIE(false)) {
    ...
    auto MaybeName = partitionCUDie(SplitData, CUDie, DebugInfoData, CUOffset,
                                    IsLittleEndian, DCU.getAddressByteSize());
    if (MaybeName)
      SplitData.MaybeLinkageName = MaybeName;
  }
  return SplitData;
}

void save(const ArrayRef<std::pair<StringRef, MCObjectProxy>> &map) {
  llvm::outs() << " HERE = " << map.size() << "\n";
  std::error_code EC;
...
Expected<AbbrevAndDebugSplit> MCCASBuilder::splitDebugInfoAndAbbrevSections() {
  ...
  for (auto [CUData, AbbrevOffset] :
    ...
    if (auto E = setHeaderLength(SplitData->DebugInfoCURefData, 0,
    ...
    if (SplitData->MaybeLinkageName)
      LinkageNameToCAS.emplace_back(*SplitData->MaybeLinkageName, *CASObj);
  }
