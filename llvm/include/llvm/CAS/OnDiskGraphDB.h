...
public:
  ...
  ArrayRef<uint8_t> getBuffer() const {
    if (is4B()) {
      ...
      return makeArrayRef((const uint8_t *)B, sizeof(InternalRef4B) * Size);
    } else {
      auto *B = Begin.get<const InternalRef *>();
      return makeArrayRef((const uint8_t *)B, sizeof(InternalRef) * Size);
    }
  }
...
public:
  ...
  Expected<Optional<ObjectHandle>> load(ObjectID Ref);
  ...
  Optional<ObjectID> getExistingReference(ArrayRef<uint8_t> Digest) const;
