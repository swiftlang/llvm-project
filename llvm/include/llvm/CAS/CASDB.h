...
///     - identify{Blob,Node,Tree}   => UniqueIDRef + out-param:
...
///       builtin CAS:
///         - BuiltinSingleFileCAS: single file contains all CAS objects,
///           suitable for an "opaque" artifact.
///         - BuiltinFilePerObjectCAS: every CAS object in a separate file.
///           Great for debugging the implementation.
...
///
/// - InMemoryView : ObjectStore. Concrete class (not abstract) that adapts
/// another
///   ObjectStore and provides an in-memory view of it. Guarantees in-memory
///   lifetime for objects (via classes \c InMemoryBlob, \c InMemoryTree, and \c
///   InMemoryNode). Re-exports the low-level ObjectStore APIs and adds new
///   high-level APIs; uses the "Capture" out-parameters to get memory mapped
///   regions from the adapted ObjectStore. Takes on the \a Namespace of the
///   adapted ObjectStore.
///     - \c InMemoryObject. Base class for objects. Has UniqueID (Namespace /
///       hash size / hash). Probably change \a ObjectRef to wrap a non-null \c
///       InMemoryObject*.
...
public:
  ...
  void printID(raw_ostream &OS, UniqueIDRef ID) const {
    getNamespace().printID(ID, OS);
  }

  Error parseID(StringRef Reference, UniqueID &ID) const {
    return getNamespace().parseID(Reference, ID);
  }
