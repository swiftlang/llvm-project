...
private:
  ...
  public:
    ...
    Error operator()() {
      ...
      if (Magic == file_magic::cas_id)
        return verifyAndAddMachOObjectFromCAS(std::move(NewMember));
  ...
  private:
    ...
    Error verifyAndAddMachOObjectFromCAS(NewArchiveMember Member) {
      if (!Builder.C.CAS)
        return createStringError(
    ...
    Error verifyAndAddMachOObject(NewArchiveMember Member) {
      auto MBRef = Member.Contents.empty() ? Member.Buf->getMemBufferRef()
                                           : Member.getContentsBufferRef();
...
static Error createStaticLibrary(LLVMContext &LLVMCtx, const Config &C) {
  ...
  if (NewMembers.size() == 1) {
    if (C.CAS)
      return writeCASIDArchive(*C.CAS, OutputFile, NewMembers.begin()->second.getMembers(),
                               /*WriteSymtab=*/true,
