        ...
        for attrname, attrvalue in attrs.items():
            if attrname.startswith("test") and not getattr(
                ...
                if not original_testcase.NO_DEBUG_INFO_TESTCASE:
                    ...
                    if getattr(attrvalue, "__pdb_variant_enrolled__", False):
                        # Swift tests are enrolled in the pdb variant only:
                        # they should not also run under the default
                        # dwarf/dwo/dsym variants.
                        dbginfo_categories = ["pdb"]
                    elif original_testcase.TEST_WITH_PDB_DEBUG_INFO:
                    ...
                    for cat in dbginfo_categories:
                        ...
                        def test_method(self, attrvalue=attrvalue, cat=cat):
                            ...
                            if cat in test_categories.non_fatal_debug_info_categories:
                                result = {"exc": None}
                                ...
                                if result["exc"] is not None:
                                    if isinstance(result["exc"], unittest.SkipTest):
                                        raise result["exc"]
                                    self.skipTest(
                                        f"tolerated under debug_info={cat}: "
                                        f"{result['exc']!r}"
