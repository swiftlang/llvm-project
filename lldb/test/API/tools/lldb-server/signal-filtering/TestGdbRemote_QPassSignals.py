...
class TestGdbRemote_QPassSignals(gdbremote_testcase.GdbRemoteTestCaseBase):
    ...
    def expect_exit_code(self, exit_code):
        self.test_sequence.add_log_lines(
            ["read packet: $vCont;c#a8", "send packet: $W{0:02x}#00".format(exit_code)],
            True,
        )
    ...
    def ignore_signals(self, signals):
        ...
        self.test_sequence.add_log_lines(
            [
                "read packet: $QPassSignals:" + signals_str + " #00",
                "send packet: $OK#00",
            ],
            True,
        )
    ...
    def test_change_signals_at_runtime(self):
        ...
        self.set_inferior_startup_launch()
        procs = self.prep_debug_monitor_and_inferior()
        expected_signals = ["SIGSEGV", "SIGUSR1", "SIGUSR2", "SIGALRM", "SIGHUP"]
